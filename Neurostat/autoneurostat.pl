...
# - what to do with patients under 19? Exit or write a warning on the image?
# - set up a daemon
...
MAIN: 
{
  ...
  # Remove extra black space from QC
  `convert -gravity East -chop 500x0 QC.tif QC_test.tif`;
...
sub run_neurostat {
  ...
  `$neurostat_dir/stereo $temp_dir/target.img $temp_dir/target -o4 -l0 -q1.0 -v-1.0`;  # -x(offset for x-axis rotation)

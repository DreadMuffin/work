...
#		print "Check er |$check[$chk_cnt]|<br>";
	...
	# run neurostat program
	my $neurostat_dir = "/usr/local/Neurostat";
	my $minc_dir      = "/opt/mni/bin";
	my $temp_dir      = "incdectemp";
	my $minc_file     = "/users/scratch/dicne/conv/$cpr[$_]/$date[$_]/$time[$_]/$cpr[$_].$date[$_].$time[$_].mnc";
	my $output_path   = "/users/scratch/dicne/conv/$cpr[$_]/$date[$_]/$time[$_]";

	
	`./autoneurostat.pl --neurostat_dir=$neurostat_dir --minc_dir=$minc_dir --temp_dir=$temp_dir --z=4 --z=7 $minc_file $output_path`;
